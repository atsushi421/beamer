...
\begin{frame}[label=lemma6]{Lemma 6}
    \hlink{lemma6}{Lemma 6} は対のパッシブVPが $\tau_j$ に有用であるための必要条件は $D_{i}-L_{i}<D_{j}-L_{j}$ であることを示す
    \begin{lemma}[]
...
\begin{frame}{アクティブ/パッシブVPの選択戦略}
    \hlink{lemma6}{Lemma 6} より, 割り当てヒューリスティックは, 小さい $D_{i}-L_{i}$ を持つタスクを選択してアクティブ VP で実行し, 大きい $D_{i}-L_{i}$ を持つタスクをパッシブ VP で実行するようにする
\end{frame}
