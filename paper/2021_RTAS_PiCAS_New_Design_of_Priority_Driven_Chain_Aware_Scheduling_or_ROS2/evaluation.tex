...
\begin{frame}{ケーススタディ I (ユニプロセッサシステム)}
    \fitimage{
        \begin{itemize}
            \item ユニプロセッサ環境における, 特定の利用可能な予算に対するチェーンレイテンシを評価する
...
\begin{frame}{ケーススタディ I 結果}
    \fitimage{
        2 つのアプローチでのチェーンのエンドツーエンドのレイテンシ
    }{case_study1_result}
\end{frame}
...
\begin{frame}{ケーススタディ II 結果1}
    \fitimage{
        3 つのアプローチで観察されたチェーンのエンドツーエンドのレイテンシ
...
\begin{frame}{解析実行時間の比較 [結果考察]}
    \begin{itemize}
        ...
        \forme{\item ROS2-SD 分析の探索空間は可能な限り長いビジー間隔であり, 分析は全ての応答時間が収束するグローバルな固定点を繰り返し探索する~\cite{casini2019response}}
