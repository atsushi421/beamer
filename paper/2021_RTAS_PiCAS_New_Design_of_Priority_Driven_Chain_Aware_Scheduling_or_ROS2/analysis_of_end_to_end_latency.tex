...
\begin{frame}{}
    \begin{itemize}
        \item 証明は, 次の 2 つの場合を考えることによって行うことができる
    \end{itemize}
\end{frame}
...
\begin{frame}{}
    \fitimage{
        \begin{itemize}
            \item 最初のケースは, $\tau_{k}$ のチェーンが $\Phi_{i}\left(\pi_{\Gamma^{c^{\prime}}}>\pi_{\Gamma^{c}}\right)$ のチェーンよりもセマンティックな優先度が高く, 干渉タスク $\tau_{k}$ が $\Phi_{i}\left(P(\Phi)=P\left(\tau_{k}\right)\right)$ と同じ $\mathrm{CPU}$ にある場合である
...
\begin{frame}{}
    \fitimage{
        \begin{itemize}
            ...
            \item 一方, システムが過負荷で, $\Gamma^{c^{\prime}}$ のすべてのコールバックが同じ $\mathrm{CPU}$ にある場合, $\tau_{k}$ の到着間隔は, $\Gamma^{c^{\prime}}$ のすべてのコールバックの実行時間の合計よりも短くすることはできない
            \item したがって,  $\mathcal{T}_{\Gamma^{c^{\prime}}}$ は,  $\tau_{k}$ の到着期間を制限するために, これら 2 つの最大値を取る
        \end{itemize}
    }{min_period}
\end{frame}
...
\begin{frame}{}
    \fitimage{
        2 番目のケースには, 最初のケースでは考慮されない他のすべての条件が含まれる
    }{proof_case2}
\end{frame}
...
\begin{frame}{ケース (iii)の分析}
    \fitimage{
        \begin{itemize}
            \item $\tau_{k}$ が優先順位の低いエグゼキュータにある場合, 干渉は発生しない
            \item $\tau_{k}$ が同じエグゼキューターにある場合, Alg によって $\Phi_{i}$ よりも優先順位が低くなるので $\Phi_{i}$ への干渉遅延を引き起こすことはできない
        \end{itemize}
    }{alg1}
\end{frame}
